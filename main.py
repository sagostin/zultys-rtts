    ...
    if result and force != '1':
        ...
        try:
            ...
            if response_tts.audio_content:
                ...
                logging.info(
                    f"File saved as {file_path}, for text: {text}",
                    extra={'clientip': client_ip}
                )
            else:
                response = f'<HTML><HEAD/><BODY>Response = ERROR<br><HR>result = 0<br>file = ERROR<br><HR></BODY></HTML>'
                logging.error(
                    f"File FAILED to generate as {file_path}, for text: {text}",
                    extra={'clientip': client_ip}
                )

        except Exception as e:
            response = f'<HTML><HEAD/><BODY>Response = ERROR<br><HR>result = 0<br>error = {e}<br><HR></BODY></HTML>'
            logging.error(
                f"Error generating TTS for text: {text} - {e}",
                extra={'clientip': client_ip}
            )
...
def purge():
    ...
    logging.info(
        f"Purged {len(old_entries)} old recordings.",
        extra={'clientip': client_ip}
    )
...
if __name__ == '__main__':
    init_db()
    # Run app in production mode on port 8081
